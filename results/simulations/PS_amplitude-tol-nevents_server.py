...
def run(iteration):
    ...
    frequency = 10.
    path_to_res = 'simulation_data/amp-tol-nev/'
    ...
    if not os.path.exists(path_to_res+result_filename):
        ...
        all_combinations = list(product(n_events,tolerances))
        ...
        results = xr.Dataset(data_vars=dict(
                ...
                coords=dict(n_events = ('all_combination', np.array(all_combinations)[:,0]),
        ...
        for n_ev in n_events:
            ...
            #MNE prints ValueError: stc must have at least three time points, got 2
                 #Perform simulations
    #                 print(times)
            file = simulations.simulate(sources, n_trials, cpus, '%s_%s_%s_%s_%s'%(n_ev, iteration, amplitude, n_trials, version),
                                path=path, overwrite=False, sfreq=sfreq, seed=iteration)
            ...
            if sim_n_trials != n_trials:
                ...
                #except: 
                     #continue
            print(sim_n_trials)
            try:# got error ValueError: setting an array element with a sequence. The requested array has an inhomogeneous shape after 1 dimensions. The detected shape was (100,) + inhomogeneous part.
            ...
            true_init = hmp.models.hmp(hmp_data, sfreq=sfreq, event_width=event_width, cpus=cpus, distribution=distribution, shape=shape, location=0)#Initialization of the model
        ...
        #       hmp.utils.save_fit(true_estimates, 'event_probs/true_estimates_%s_%s_%s.nc'%(true_combination[0], true_combination[1],iteration))
            for tolerance in tolerances:
                results.true_trial_times.loc[j,:,:n_ev+1] = random_source_times
                results.gen_mags.loc[j, :n_ev] = true_amplitudes
                results.true_n_events.loc[j] = n_ev
                # results.names.loc[j, :n_ev+1] = name_sources
                # results.means.loc[j, :n_ev+1] = means_list
                print((n_ev, tolerance, times, means_list, iteration))

                # Estimating an HMP model
                tstart = time.time()
                test_init = hmp.models.hmp(hmp_data, sfreq=sfreq, event_width=event_width, cpus=cpus, distribution=distribution, shape=shape)#Initialization of the model

                fit = test_init.fit(verbose=False, tolerance=tolerance)
