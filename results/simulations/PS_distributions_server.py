...
def run(iteration):
    ...
    frequency, amplitude = 10., 3e-07
    path_to_res = 'distribution_results/%s' %amplitude
    ...
    for true_combination in distributions:
        ...
        sim_n_trials = 0
        x = 0
        while sim_n_trials != n_trials and x <5:#Sometimes a few trials gets rejected,e.g. longer than epoch length... annoying for the comparison with simulated trials
        ...
        for test_combination in distributions:
            ...
            try:
                ...
                if fit is not None:
                    ...
                    results.true_trial_times.loc[j] = random_source_times
                    test_times = test_init.compute_times(test_init, fit, duration=True, add_rt=True).values
